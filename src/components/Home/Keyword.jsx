...
export default function KeywordsSection({ keywords = [] }) {
  ...
  return (
    <section className="bg-[#212121] text-[#f9e3aa] py-6 overflow-hidden">
      <div
        ref={containerRef}
      ...
      >
        {scrollingKeywords.map((word, i) => (
          <span
            key={i}
            className="mx-10 text-sm sm:text-base md:text-xl lg:text-2xl tracking-widest hover:text-[#cb972b] transition-colors duration-300 cursor-pointer"
          >
            {word}
          </span>
        ))}
